...
class Maze:
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        for waypoint in self.filter_valid_waypoints().get(cur_shape, []):
            if cur_waypoint == waypoint:
                continue
    
            new_position = (waypoint[0], waypoint[1], cur_shape)
    ...
    def is_valid_move(self, start, end):
        ...
        num_shapes = len(self.alien.get_shapes())
        
        if not (0 <= start_shape_idx < num_shapes and 0 <= end_shape_idx < num_shapes):
            return False
        alienk = self.create_new_alien(end_x, end_y, end_shape_idx)
        if does_alien_touch_wall(alienk, self.walls):
            return False
        ...
        if start_shape_idx == end_shape_idx:
            ...
            return not does_alien_path_touch_wall(alienk, self.walls, (end_x, end_y))
        else:
            if start_x == end_x and start_y == end_y:
                allowed_transitions = [(0, 1), (1, 0), (1, 2), (2, 1)]
                if (start_shape_idx, end_shape_idx) in allowed_transitions:
                    alienk = self.create_new_alien(start_x, start_y, end_shape_idx)
