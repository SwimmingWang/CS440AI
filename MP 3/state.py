...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for d_i, d_j in directions:
            ...
            if 0 <= new_i < 3 and 0 <= new_j < 3:
                ...
                nbr_states.append(EightPuzzleState(new_state, goal = self.goal, 
                                                   dist_from_start=self.dist_from_start + 1, 
                                                   use_heuristic=self.use_heuristic,zero_loc = (new_i, new_j)))
        return nbr_states
    ...
    def compute_heuristic(self):
        ...
        for row in range(3):
            for col in range(3):
                cur = self.state[row][col]
                if cur != 0:
                    _row = cur // 3
                    _col = cur % 3
                    total += manhattan((row, col), (_row, _col))
        return total
