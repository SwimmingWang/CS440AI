...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        empty_row, empty_col = self.zero_loc
        for dr, dc in directions:
            new_row, new_col = empty_row + dr, empty_col + dc
            if (0 <= new_row < row) and 0 <= new_col < col:
                new_state = copy.deepcopy(self.state)
                new_state[new_row][new_col] = self.state[empty_row][empty_col]
                new_state[empty_row][empty_col] = self.state[new_row][new_col]
                new_zero_loc = (new_row, new_col)
                new_puzzle = EightPuzzleState(new_state, goal=self.goal, dist_from_start=self.dist_from_start+1,
                                              use_heuristic=self.use_heuristic, zero_loc=new_zero_loc)
                nbr_states.append(new_puzzle)
    ...
    def compute_heuristic(self):
        ...
        for row in range(3):
            for col in range(3):
                ...
                total += manhattan(cur_pos, goal_pos)
        return total
